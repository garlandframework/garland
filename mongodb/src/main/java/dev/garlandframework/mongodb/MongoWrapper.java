...
public class MongoWrapper implements AutoCloseable {
    ...
    private static List<Bson> buildFilters(Object example) {
        return buildFilters(example, "", true);
    }

    private static List<Bson> buildFilters(Object example, String prefix, boolean topLevel) {
        ...
        while (current != null && current != Object.class) {
            for (Field field : current.getDeclaredFields()) {
                ...
                try {
                    ...
                    String fieldName = (topLevel && MongoIdExtractor.isIdField(field)) ? "_id" : field.getName();
                    String key = prefix.isEmpty() ? fieldName : prefix + "." + fieldName;
                    if (isScalar(value)) {
                        filters.add(Filters.eq(key, value));
                    } else {
                        filters.addAll(buildFilters(value, key, false));
                    }
    ...
    private static boolean isScalar(Object value) {
        ...
        return c.isPrimitive() || c.isEnum() || c.getName().startsWith("java.");
    }
